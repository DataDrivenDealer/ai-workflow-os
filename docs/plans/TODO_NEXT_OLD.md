...
**文档ID**: TODO_NEXT  
**创建日期**: 2026-02-02  
**最后更新**: 2026-02-02T02:30:00Z（基于漂移审计重置）  
**状态**: ACTIVE  
...
1. 修改[kernel/os.py](../../kernel/os.py#L12-L18)所有导入:
   ```python
   # 修改前
...
3. 同样修改kernel/mcp_stdio.py, kernel/config.py等所有模块
           ...
           for node in ast.walk(tree):
               if isinstance(node, ast.ImportFrom):
                   assert node.module.startswith("kernel."), \
                       f"{py_file}: {node.module} 应该以 'kernel.' 开头"
   ```
...
**验证证据**:
- ✅ `python -m pytest kernel/tests/test_imports.py -q` 通过（1 passed）
- ✅ `python -m pytest kernel/tests/ -q` 通过（173 passed）
- ✅ `python -m pyright kernel/` 通过（0 errors）
...
**执行结果**:
- ✅ 已创建文档: [docs/SYSTEM_INVARIANTS.md](../../docs/SYSTEM_INVARIANTS.md)
...
**问题描述**:
[kernel/state_store.py](../../kernel/state_store.py)并发控制已实现但测试覆盖仅89%，需补充边界情况测试。

**Acceptance Criteria**:
- [x] test_state_store_concurrency.py覆盖率>95%（本地测试通过）
- [x] 测试场景：同时读写、死锁检测、超时机制
- [x] 性能测试：1000次并发写入无数据损坏
...
**执行结果**:
- ✅ 新增高并发写入测试（1000 keys）
- ✅ `python -m pytest kernel/tests/test_state_store_concurrency.py -q` 通过（5 passed）
- ⏳ 跨平台验证待CI
...
**执行结果**:
- ✅ 已删除 .github/workflows/ci.yaml（保留ci.yml作为唯一CI）
- ⏳ GitHub Actions需在远端确认
 - ✅ 本地修复：移除 submodules 递归 + DGSF可用性检测 + governance-check 绝对导入
...
# 检查退出码
$LASTEXITCODE  # 应该是0 (pass), 1 (warnings), 或 2 (errors)
```

**执行结果（G2）**:
- ✅ 已创建 [scripts/run_gate_g2.py](../../scripts/run_gate_g2.py)
- ⚠️ 当前输出：1 warning（type_hints），0 errors

**执行结果（G3）**:
- ✅ 已创建 [scripts/run_gate_g3.py](../../scripts/run_gate_g3.py)
- ⚠️ 当前输出：1 warning（performance report missing），0 errors

**执行结果（G4）**:
- ✅ 已创建 [scripts/run_gate_g4.py](../../scripts/run_gate_g4.py)
- ⚠️ 当前输出：1 warning（backtest report missing），0 errors

**执行结果（G5）**:
- ✅ 已创建 [scripts/run_gate_g5.py](../../scripts/run_gate_g5.py)
- ✅ 当前输出：0 warning，0 errors

**执行结果（G6）**:
- ✅ 已创建 [scripts/run_gate_g6.py](../../scripts/run_gate_g6.py)
- ⚠️ 当前输出：1 warning（release notes missing），0 errors

---

## 🟡 P2 Tasks（质量改进 - 可defer）
...
**当前应执行**: P0-1（修复kernel导入路径）  
**下一步**: P0-2 → P1-1 → P1-2 → P1-3 → P1-4 → P1-5（分3天） → P2任务
